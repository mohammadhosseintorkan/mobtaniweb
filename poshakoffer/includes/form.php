...
EOT;
        ...
        private function number($type = 'number', $label, $name, $value, $helper, $min, $step, $options){
            if( ! isset($name) )
                $name = $type;
            if( ! isset($label) )
                $label = $name;
            $output = <<< EOT
                    <label for = "{$name}"  class = "form-label">{$label}</label><br>
                    <input type = "{$type}" name = "{$name}" id = "{$name}" value = "{$value}" min = "{$min}" step = "{$step}" class = "form-control" {$options}>
EOT;
            $this -> template( $output, $helper );
        }
...
EOT;
        ...
        public function numberbox( $label = null, $name = null, $value = 0, $helper = null, $min = null, $step = null, $options = '' ){
            $this->number('number', $label, $name, $value, $helper, $min, $step, $options);
        }
